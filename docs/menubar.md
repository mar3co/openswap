...
`MenuBarSettings` is `menubar_settings.json` (title, refresh, Claude auto on/off, ChatGPT suggestions, kickoff). General holds menu-bar display, confirmation, and refresh controls. Automation names each surface explicitly: **Auto-switch Claude accounts**, **Suggest ChatGPT account switches**, and **Auto-switch Codex CLI accounts**. ChatGPT suggestions use Codex quota but never restart the desktop app without approval. They cannot run with live Codex CLI rotation; the Codex control is disabled with an explanation while ChatGPT suggestions are on. The threshold and next-account strategy form one **Shared rotation policy** in `settings.json` via `openswap config`. Cooldown / last switch live in `autoswitch_state.json`. Changing strategy from the extra calls `set_setting` then `_restart_engine` so the running engine reloads policy.
