...
When switching is on, the extra probes the allowlisted ChatGPT app and the transaction’s complete Codex configuration policy on a worker the first time the ChatGPT tab is shown: never codesign or process inspection on the AppKit thread. Typed states are `checking`, `unsupported`, `missing`, `invalid`, `stopped`, and `running`. A fresh stopped observation uses **Switch and open ChatGPT**; a fresh running observation uses **Restart ChatGPT**. Running/stopped observations and retryable operational probe failures expire after five seconds. Installation, process state, selected credentials, and relaunch do not verify the profile ChatGPT actually loaded.
