...
`pyproject.toml` sets `-n auto`. About 2860 tests. CI runs on macOS, and still on Ubuntu and Windows as a test farm (we do not ship those platforms).
...
| --- | --- |
| `tests/test_menubar.py` | No. Drive `MenuBarApp` via `tests/menubar_harness.py`. |
